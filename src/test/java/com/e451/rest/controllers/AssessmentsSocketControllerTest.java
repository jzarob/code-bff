...
public class AssessmentsSocketControllerTest {
    ...
    public void setup() {
        ...
        questionAnswerList = Arrays.asList(
                ...
                new QuestionAnswer("Q3", "body3", "answer3", "3")
        );
    }
    ...
    public void whenCreateQuestionAnswer_returnNewQuestionAnswer() {
        ...
        questionAnswerResponse.setQuestionAnswers(Arrays.asList(this.questionAnswerList.get(0)));

        ResponseEntity<QuestionAnswerResponse> responseEntity = ResponseEntity.status(HttpStatus.CREATED)
                .body(questionAnswerResponse);

        NewQuestionEvent newQuestionEvent = new NewQuestionEvent("Q1", "body1", null);

        when(questionAnswerService.createQuestionAnswer(any(QuestionAnswer.class), any(String.class))).thenReturn(responseEntity);
    ...
    public void whenUpdateQuestionAnswer_returnUpdatedQuestionAnswer() {
        ...
        ResponseEntity<QuestionAnswerResponse> responseEntity = ResponseEntity.status(HttpStatus.ACCEPTED).body(questionAnswerResponse);

        when(questionAnswerService.updateQuestionAnswer(any(QuestionAnswer.class), any(String.class))).thenReturn(responseEntity);
